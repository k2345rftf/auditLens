...
0) ⛔ НЕ ВЫДУМЫВАЙ номера ФЗ/постановлений и проценты субсидий. Номер указывай
   ТОЛЬКО если он есть в фактах/источниках; иначе — название программы без номера.

1) Конкретные ПРОГРАММЫ называй полностью:
   ✅ «Программа "Семейная ипотека"» (номер ФЗ — если есть в источнике)
   ❌ «Господдержка ипотеки»

2) Цифры (ставки, % субсидии, лимиты) — ТОЛЬКО из фактов, не оценочно.
...
async def generate(ctx: NarrativeContext) -> str:
    ...
    if not box_gate(prog_facts, ctx.entities, min_facts=2, require_multi_bank=False):
        return ""
